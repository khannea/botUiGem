...
export class EventsStore {
  constructor(private proxyStore: ProxyStore, private apiStore: ApiStore) {}

  readonly initialized = lazyObservablePromise(async () => {
    await Promise.all([this.proxyStore.initialized, this.apiStore.initialized]);
  ...
  private addInitiatedRequest(request: InputInitiatedRequest) {
    try {
      ...
      while (i > 0) {
        ...
        if (
          eventHandler[i] &&
          eventHandler[i].request &&
          eventHandler[i].request.parsedUrl.href.startsWith(
            "https://api.stepn.com/run/orderlist"
          )
        ) {
  ...
  private addCompletedRequest(request: InputCompletedRequest) {
    try {
      ...
      if (existingEventIndex >= 0) {
        (
          this.events[existingEventIndex] as HttpExchange
        ).updateFromCompletedRequest(request);
      } else {
  ...
  async loadFromHar(harContents: {}) {
    const { requests, responses, aborts, tlsErrors } = await parseHar(
      harContents
